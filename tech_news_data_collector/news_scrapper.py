...
def save_db(dict_new_detail):
    with MongoClient() as client:
        ...
        db.news_collection.find_one_and_replace({'url': dict_new_detail['url']}, dict_new_detail, upsert=True)
...
def fix_line_breaks(string):
    return [item_list.strip() for item_list in string if len(item_list.strip()) > 0]
...
def get_news_details(url, total, current):
    ...
    new_detailed['datetime'] = selector.css(".tec--timestamp__item time ::attr(datetime)").get() or ''
    ...
    if len(selector.css("div.tec--toolbar__item").getall()) == 2:
        share, comments_count = selector.css("div.tec--toolbar__item ::text").re('\d+')
    else:
        comments_count = selector.css("#js-comments-btn::attr(data-count)").get()
        share = 0
    new_detailed['comments_count'] = comments_count
    new_detailed['share'] = share
    new_detailed['summary'] = ''.join(selector.css(".tec--article__body > p:first_child *::text").getall()) or ''
    
    source = ''.join(selector.css(".z--mb-16 a ::text").getall()).strip().split('\n')
...
def scrape(n=1):
    ...
    for number in range(1, n + 1):
        ...
        selector = parsel.Selector(content)
        news_urls = selector.css(".tec--list--lg .tec--list__item .tec--card__thumb__link::attr(href)").getall()
        for img_url in news_urls:
            urls_to_scrape.append(img_url)
       
    for url in urls_to_scrape:
        save_db(get_news_details(url, len(urls_to_scrape), urls_to_scrape.index(url) + 1))
