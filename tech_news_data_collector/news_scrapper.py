...
def requestLinks(searchPages=1):
    ...
    while aux < searchPages:
        ...
        links = (
            links
            + selector.css(".tec--list__item .tec--card__title a::attr(href)").getall()
        )
        ...
        arrayInfos.append(
            {
                "url": link,
                "title": (
                    selector.css(".tec--article__header__title::text").get() or ""
                ).strip(),
                "timestamp": (
                    selector.css(".tec--timestamp time::attr(datetime)").get() or ""
                ),
                "writer": (
                    (selector.css(".tec--author__info__link::text").get() or "").strip()
                ),
                "shares_count": (
                    selector.css(".tec--toolbar .tec--toolbar__item::text").re_first(
                        r"\d{1,}"
                    )
                    or 0
                ),
                ...
                "sources": [
                    source.strip()
                    for source in (selector.css(".z--mb-16 div a::text").getall() or [])
                ],
                "categories": [
                    categorie.strip()
                    for categorie in (
                        selector.css("#js-categories a::text").getall() or []
                    )
                ],
            }
        )
