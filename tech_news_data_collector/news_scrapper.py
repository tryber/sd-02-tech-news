...
def save_db(dict_new_detail):
    with MongoClient() as client:
        ...
        db.news_collection.find_one_and_replace(
            {"url": dict_new_detail["url"]}, dict_new_detail, upsert=True
        )
...
def fix_line_breaks(string):
    return [
        item_list.strip() for item_list in string if len(item_list.strip()) > 0
    ]
...
def get_news_details(url, total, current):
    ...
    new_detailed["timestamp"] = get_simple_text(
        selector,
    ...
    source = (
        "".join(selector.css(".z--mb-16 a ::text").getall())
        .strip()
        .split("\n")
    )
...
def scrape(n=1):
    ...
    for url in urls_to_scrape:
        save_db(
            get_news_details(
                url, len(urls_to_scrape), urls_to_scrape.index(url) + 1
            )
        )
